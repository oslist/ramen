...
macro_rules! call_handler {
    ($handler:expr) => {
        ...
        unsafe {
            ...
            call {}
            mov rsp, rax
        ", const INTERRUPT_STACK.as_u64(), sym apic::local::end_of_interrupt, sym $handler, sym process::manager::switch, out("rax") _, out("rbx") _, out("rcx") _, out("rdx") _, out("rsi") _, out("rdi") _,  out("r8") _, out("r9") _, out("r10") _, out("r11") _, out("r12") _, out("r13") _, out("r14") _, out("r15") _);
        }
    };
}
...
pub extern "x86-interrupt" fn h_21(
    ...
    call_handler!(notify::<0x21>);
}

pub extern "x86-interrupt" fn h_2c(
    _stack_frame: &mut x86_64::structures::idt::InterruptStackFrame,
) {
    call_handler!(notify::<0x2c>);
}
...
fn notify<const N: usize>() {
    if let Some(a) = NOTIFY_ON_INTERRUPT.lock().get(&N) {
        for pid in a {
